"""
History Logger
- Deduplication: same alert logged at most once per 15 seconds
- total_potential_pct only counts alerts with potential_pct >= 0.2%
- All timestamps in UTC
- Writes: data/history_YYYY-MM-DD.json  +  alert_logs.txt
"""
...
DEDUP_TTL = 15        # seconds between logging same alert
...
def _write_session_footer():
    ...
    _append_log(f"\n{'-'*72}\n  SESSION ENDED: {ts}  |  Duration: {h:02d}:{m:02d}:{s:02d}  |  Alerts: {_session_alerts}\n{'-'*72}\n")
        ...
        if a.get("spread_alert"):
            ...
            if now - _dedup.get(key, 0) >= DEDUP_TTL:
                ...
                rec = {
                    ...
                    "trust_level":    _worst_trust(a.get("trust_map",{}), buy_ex, sell_ex),
                }
                new_records.append(rec)
                _session_alerts += 1
                ts_s = datetime.fromtimestamp(now, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                _append_log(f"[{ts_s} UTC] SPREAD  {sym:<6} +{spread_pct:.4f}%  LONG {buy_ex} @ ${a.get('min_price',0):.4f}  SHORT {sell_ex} @ ${a.get('max_price',0):.4f}  trust={rec['trust_level']}")

        # ── Funding alerts ────────────────────────────────────────────
        for opp in a.get("funding_opportunities", []):
            if not opp.get("alert"): continue
            ...
            if now - _dedup.get(key, 0) >= DEDUP_TTL:
                ...
                rec = {
                    ...
                    "trust_level":    opp.get("trust_level", "medium"),
                }
                new_records.append(rec)
                _session_alerts += 1
                ts_s = datetime.fromtimestamp(now, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                _append_log(f"[{ts_s} UTC] FUNDING {sym:<6} +{diff_pct:.4f}%/8h  ({annual:.1f}% annual)  SHORT {short_ex}  LONG {long_ex}  trust={rec['trust_level']}")
...
def _worst_trust(trust_map, ex_a, ex_b):
    t_a = trust_map.get(ex_a, "medium"); t_b = trust_map.get(ex_b, "medium")
...
def compute_analytics(records: list, coin_filter: str = None, type_filter: str = None) -> dict:
    ...
    # Only count toward gross % if >= MIN_GROSS (0.2%)
    ...
    for r in records: by_type[r.get("type", "spread")] += 1

    by_pair: dict = {}
    for r in records:
        key = f"{r.get('buy_exchange','?')} → {r.get('sell_exchange','?')}" if r["type"] == "spread" else f"{r.get('long_exchange','?')} / {r.get('short_exchange','?')}"
        if key not in by_pair: by_pair[key] = {"count": 0, "total_pct": 0}
        by_pair[key]["count"] += 1
        p = r.get("potential_pct", 0) or 0
        if p >= MIN_GROSS: by_pair[key]["total_pct"] = round(by_pair[key]["total_pct"] + p, 5)

    by_trust = {"high": 0, "medium": 0, "low": 0}
    for r in records:
        t = r.get("trust_level", "medium"); by_trust[t] = by_trust.get(t, 0) + 1

    buckets: dict = {}
    for r in records:
        hour = datetime.fromtimestamp(r["ts"], tz=timezone.utc).strftime("%Y-%m-%d %H:00")
        if hour not in buckets: buckets[hour] = {"time": hour, "count": 0, "potential": 0}
        buckets[hour]["count"] += 1
        p = r.get("potential_pct", 0) or 0
        if p >= MIN_GROSS: buckets[hour]["potential"] = round(buckets[hour]["potential"] + p, 5)
